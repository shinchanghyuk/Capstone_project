...
public class TeamBoardContentActivity extends AppCompatActivity {
    ...
    private DatabaseReference team_database, team_database2, comment_database, comment_database2,
            user_database, manager_database, recomment_database, report_database; // 파이버에시스 연결(경로) 선언
    private TextView matching_tv, place_tv, date_tv, person_tv, ability_tv, content_tv, title_tv, name_tv; // 텍스트 뷰들 선언
    private Button update_btn, delete_btn, list_btn, reply_btn, report_btn, process_btn; // 버튼들 선언
    private String matching, day, title, content, ability, name, person, current_user, boardnumber, place,
            commentkey, writeTime, reply, fcmUrl, serverKey, fcmToken, alarm_content, alarm_title, current_uid; // 쓰이는 문자열 선언
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        report_btn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ReportDialog dialog = new ReportDialog(TeamBoardContentActivity.this);
                dialog.operation("board", "team");
                // 신고 다이얼로그를 띄움
            }
        });
    ...
    private void init() {
        ...
        // team 키에 있는 boardnumber와 받아온 boardnumber를 비교
        ...
        adapter = new CommentAdapter(comment_arrayList, this, "team");
    ...
    public void boardDelete() {
        team_database2 = team_database.child(boardnumber);
        ...
        team_database2.removeValue();
        manager_database.removeValue();
        // 이 게시물에 대한 데이터들을 지움

        Intent intent = new Intent(TeamBoardContentActivity.this, TeamBoardActivity.class);
        startActivity(intent);
        // 데이터 정보를 지운 후 팀 홍보 게시판 화면으로 이동
}
    ...
    private void commentInsert() {
        ...
        if (reply.isEmpty()) {  // 댓글 내용을 작성하지 않고 버튼을 눌렀을 때
            ...
            if (!(uid.equals(current_uid))) { // 게시물을 작성자인 사용자가 댓글을 적었을때 제외한 나머지에게 알림을 전송
                query.addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
        ...
        report_database.orderByChild("reportnumber").equalTo(reportnum).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    ...
                    Intent intent = new Intent(TeamBoardContentActivity.this, ReportHistoryActivity.class);
                    startActivity(intent);
