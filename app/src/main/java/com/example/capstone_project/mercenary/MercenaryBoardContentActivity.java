...
public class MercenaryBoardContentActivity extends AppCompatActivity {
    ...
    private DatabaseReference mercenary_database, mercenary_database2, comment_database, comment_database2,
    ...
    private String matching, day, title, content, ability, starttime, endtime, person,
            current_user, boardnumber, place, type, commentkey, reply, fcmUrl, serverKey, fcmToken,
            alarm_content, alarm_title, current_uid, writeTime; // 쓰이는 문자열 선언
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        report_btn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ReportDialog dialog = new ReportDialog(MercenaryBoardContentActivity.this);
                dialog.operation("board", "mercenary");
                // 신고 다이얼로그를 띄움
            }
        });
        ...
        process_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ConfirmDialog dialog = new ConfirmDialog(MercenaryBoardContentActivity.this);
                dialog.operation("process", "mercenary");
    ...
    private void init() {
        ...
        Query query = mercenary_database.orderByChild("boardnumber").equalTo(boardnumber);
        // mercenary 키에 있는 boardnumber와 받아온 boardnumber를 비교
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if (current_uid.equals(uid)) { // 현재 사용자가 작성 사용자 일때
                    ...
                    if(matching.equals("매칭완료")) { // 이 게시물이 매칭완료 된 게시물이라면
                        matching_tv.setTextSize(13); // 텍스트 뷰 글자의 크기를 13으로 설정
                        matching_btn.setText("매칭취소"); // 매칭완료 버튼을 매칭취소 버튼으로 변경
                    }else if(matching.equals("매칭 중")){ // 이 게시물이 매칭 중인 게시풀이라면
                        matching_tv.setTextSize(15); // 텍스트 뷰 글자의 크기를 15으로 설정
        ...
        adapter = new CommentAdapter(comment_arrayList, this, "mercenary");
    ...
    public void matchingChange() {
        ...
        mercenary_database2 = mercenary_database.child(boardnumber);
        // 해당 게시물을 변경하기 위한 파이어베이스 경로 설정

        Map<String, Object> matchingOk = new HashMap<>();
        matchingOk.put("matching", matching);
        mercenary_database2.updateChildren(matchingOk);
    ...
    public void boardDelete() {
        ...
        mercenary_database2.removeValue();
        ...
        report_database.orderByChild("reportnumber").equalTo(reportnum).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    ...
                    Intent intent = new Intent(MercenaryBoardContentActivity.this, ReportHistoryActivity.class);
                    startActivity(intent);
