...
public class MercenaryBoardAdapter extends RecyclerView.Adapter<MercenaryBoardAdapter.ViewHolder> {
    private ArrayList<MercenaryBoardItem> arrayList;
    private String boardnumber; // 게시물 번호를 나타내는 문자열 선언
    private Context context;

    public MercenaryBoardAdapter(ArrayList<MercenaryBoardItem> arrayList, Context context) {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, final int position) {
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) { // 리사이클러뷰 목록을 클릭했을 때
                ...
                // 해당 게시물번호를 변수에 넣음
                Intent intent = new Intent(v.getContext(), MercenaryBoardContentActivity.class);
                intent.putExtra("boardnumber", boardnumber);
                context.startActivity(intent);
                // 해당 게시물 번호를 가지고 MercenaryBoardContentActivity로 이동
            }
        });
    }
    @Override
