...
public class RelativeBoardActivity extends AppCompatActivity {
    ...
    private RecyclerView.Adapter relative_adapter, search_adapter; // 리사이클러뷰 어댑터 선언
    private ArrayList<RelativeBoardItem> search_arrayList, relative_arrayList; // 아이템 담을 배열리스트 선언
    private FirebaseDatabase firebaseDatabase; // 파이어베이스 데이터베이스 객체 선언
    private DatabaseReference relative_database, user_database, manager_database; // 파이버에시스 연결(경로) 선언
    private FirebaseAuth auth; // 파이어베이스 인증 객체
    private String sort, sort_search, current_uid, realarm, manager_uid; // 쓰이는 문자열 선언
                ...
                query.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                            ...
                            // 데이터베이스에서 현재 사용자의 realarm 값을 가져옴
                        }

                        if (realarm.equals("o")) { // 이 사용자가 상대매칭 알림을 받는 사용자 일때(realarm 값이 o일 때)
        ...
        search_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (sort.equals("검색") || sort_search.isEmpty()) { // 키워드 설정 및 검색 내용이 비어있다면 Toast 메세지 전송
                    ...
                    relative_database.orderByChild("writetime").addListenerForSingleValueEvent(new ValueEventListener() {
                        // 게시물 작성 시간으로 정렬
                        @Override
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            ...
                            for (DataSnapshot snapshot : dataSnapshot.getChildren()) { // 반복문으로 데이터리스트를 추출
                                ...
                                if (sort.equals("지역")) { // 지역으로 검색할 때
                                    if (relativeBoardItem.getPlace().contains(sort_search)) {
                                        search_arrayList.add(relativeBoardItem);
                                        // 사용자가 입력한 내용이 데이터베이스에 포함되어 있을 때 배열리스트에 추가
                                    }
                                } else if (sort.equals("제목")) { // 제목으로 검색할 때
                                    if (relativeBoardItem.getTitle().contains(sort_search)) {
                                        search_arrayList.add(relativeBoardItem);
                                        // 사용자가 입력한 내용이 데이터베이스에 포함되어 있을 때 배열리스트에 추가
                                    }
                                } else if (sort.equals("일자")) { // 일자로 검색할 때
                                    if (relativeBoardItem.getDay().contains(sort_search)) {
                                        search_arrayList.add(relativeBoardItem);
                                        // 사용자가 입력한 내용이 데이터베이스에 포함되어 있을 때 배열리스트에 추가
                                    }
                                } else if (sort.equals("작성자")) { // 작성자로 검색할 때
                                    if (relativeBoardItem.getUser().contains(sort_search)) {
                                        search_arrayList.add(relativeBoardItem);
                                        // 사용자가 입력한 내용이 데이터베이스에 포함되어 있을 때 배열리스트에 추가
                                    }
                                }
                            ...
                            if (search_arrayList.size() == 0) { // 사용자의 검색 조건에 해당하는 게시물이 없을 때
                                ...
                                recyclerView.setAdapter(relative_adapter); // 리사이클러뷰에 상대매칭 게시판에 등록 된 데이터들을 담음
    ...
    private void init() {
        ...
        relative_arrayList = new ArrayList<>();
        // 상대매칭 게시판에 데이터들을 담을 배열리스트 생성
        ...
        // relative 키 값에 접근하기 위한 파이어베이스 경로 설정
        Query query = relative_database.orderByChild("writetime"); // 사용자가 게시물을 작성한 시간으로 정렬

        query.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                relative_arrayList.clear(); // 상대매칭 게시판에 등록된 데이터들을 담을 배열리스트 초기화

                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {   // 반복문으로 데이터리스트를 추출
                    RelativeBoardItem relativeBoardItem = snapshot.getValue(RelativeBoardItem.class);
                    // RelativeBoardItem 객체에 데이터를 담음
                    relative_arrayList.add(relativeBoardItem); // 모든 상대매칭 게시물의 데이터들을 배열리스트에 추가
                }
                Collections.reverse(relative_arrayList); // 배열리스트 내림차순으로 정렬
                relative_adapter.notifyDataSetChanged(); // 리스트 저장 및 새로고침
            }
        ...
        relative_adapter = new RelativeBoardAdapter(relative_arrayList, this);
        search_adapter = new RelativeBoardAdapter(search_arrayList, this);
        // RelativeBoardAdapter 객체들을 생성
